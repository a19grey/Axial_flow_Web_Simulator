...
export async function runBiotSavart(G, B, job, { onProgress, signal } = {}) {
  ...
  B.bg.bs = d.createBindGroup({
    ...
    entries: [B.bsp, B.segBuf, B.H].map((b, i) => ({ binding: i, resource: { buffer: b } }))
  });
  ...
  const h = job.hm * 1e-3;
  // Finite-core radius regularizes the 1/r singularity at the wire itself.
